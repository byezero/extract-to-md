...
它可以在可用时调用 [Microsoft MarkItDown](https://github.com/microsoft/markitdown)，同时增加了更适合本地文档处理的兜底逻辑：PDF 按页文本层抽取、扫描件 OCR、图片 OCR、PPT 渲染后 OCR、Excel 行数限制、环境诊断等。
...
| `.pdf` 有文本层 | 按页使用 `pdftotext -layout` 保留布局 | Poppler 的 `pdfinfo`、`pdftotext` |
| 扫描版 `.pdf` | 按页渲染图片后 OCR | Poppler 的 `pdfinfo`、`pdftoppm`、`tesseract` |
...
- `markitdown` 是否可用
- `pdfinfo` / `pdftotext` / `pdftoppm` 是否可用
- `tesseract` 是否可用
...
`auto` 是默认模式：每页先尝试文本层，文本太少或疑似乱码时只对该页 OCR。输出里会记录每页使用了 `text` 还是 `ocr`。
...
- PDF 按页优先用 `pdftotext -layout` 抽文本，单页文本太少或疑似乱码时只 OCR 该页
- 扫描版 PDF 可以自动或强制 OCR
- 图片直接用 Tesseract OCR
...
如果报 `Required tool not found: pdfinfo` 或 `pdftoppm`，也是安装同一个 Poppler 包。
